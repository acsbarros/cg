...
void main() {
    ...
    // Inicializar componentes de luz
    vec3 result = vec3(0.0);
    
    // Processar Luz 1 (Luz Principal)
    ...
    // Adicionar um pequeno brilho baseado no tempo (efeito de animação)
...
def create_cube_data():
    ...
    # Mapeamento de cada triângulo para sua face
    ...
    for i, idx in enumerate(indices):
        ...
        face_idx = i // 6  # Cada face tem 6 vértices (2 triângulos)
...
# ==================== CLASSE PRINCIPAL ====================
class OpenGLApp:
    def __init__(self, width=1024, height=768, title="OpenGL com Iluminação - UNILAB"):
        self.width = width
        self.height = height
        self.title = title
        ...
        glfw.window_hint(glfw.OPENGL_FORWARD_COMPAT, GL_TRUE)  # Importante para macOS
        ...
        # Configurar callbacks ANTES de qualquer outra operação
        ...
        # Habilitar depth testing (ESSENCIAL para 3D)
        glEnable(GL_DEPTH_TEST)
        glDepthFunc(GL_LESS)
        
        # Configurar blending (para transparência - opcional)
        glEnable(GL_BLEND)
    ...
    def create_shader_program(self):
        ...
        try:
            ...
            # Tentar mostrar erro mais detalhado
            import traceback
            traceback.print_exc()
            sys.exit(1)
    ...
    def update_lights(self):
        ...
        # Atualizar tempo baseado no delta time para animação suave
        current_time = glfw.get_time()
        delta_time = current_time - self.last_time
        self.last_time = current_time
        self.time += delta_time if delta_time < 0.1 else 0.016  # Limitar delta time
        
        # Luz 1: Luz principal fixa (branca)
        ...
        # Luz 2: Luz rotativa ao redor do objeto (vermelha)
        radius = 2.5
        angle = self.time * 1.5  # Velocidade de rotação
        light2_pos = np.array([
        ...
        # Luz 3: Luz pulsante (azul) que se move verticalmente
        light3_pos = np.array([
            -2.0,
    ...
    def render(self):
        ...
        # Atualizar matrizes
        model, view, projection = self.update_matrices()
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("✨ Luzes:")
    print("   💡 Luz 1: Branca fixa (direita/cima/frente)")
    print("   🔴 Luz 2: Vermelha rotativa ao redor do objeto")
    print("   🔵 Luz 3: Azul pulsante com movimento vertical")
    print("=" * 60)
