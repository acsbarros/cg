...
# Vertex shader source code com suporte a matriz de modelo e projeção
...
class Transformations:
    ...
    def __init__(self):
        ...
        self.rotation_angle = 0.0  # em graus
        self.scale_x = 1.0
        self.scale_y = 1.0
    ...
    def get_model_matrix(self):
        ...
        # (a ordem importa! Primeiro escala, depois rotaciona, depois translada)
...
def get_view_matrix(eye_pos=np.array([0.0, 0.0, 1.0]), target_pos=np.array([0.0, 0.0, 0.0]), up=np.array([0.0, 1.0, 0.0])):
    """Cria uma matriz de view"""
    # vetor direção da câmera
    forward = eye_pos - target_pos # n = eye_pos - target_pos    
    # normalizar o vetor direção
    forward = forward / np.linalg.norm(forward) # nx,ny,nz
...
def key_callback(window, key, scancode, action, mods):
    ...
    if action == glfw.PRESS or action == glfw.REPEAT:
        ...
        # Teclas de escala (W e S para X, A e D para Y)
        elif key == glfw.KEY_W:
        ...
        # Reset (tecla R)
            ...
            print(f"Translação: ({transforms.translation_x}, {transforms.translation_y})")
            print(f"Rotação: {transforms.rotation_angle}°")
            print(f"Escala: ({transforms.scale_x}, {transforms.scale_y})")
            print("-------------------------------\n")
...
def main():
    ...
    # Armazenar dados na janela para uso nos callbacks
    # Precisamos armazenar tanto o shader quanto as transformações
    ...
    while not glfw.window_should_close(window):
        ...
        # Obter e aplicar a matriz de modelo atual
